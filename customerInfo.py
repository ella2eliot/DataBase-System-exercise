...
def frame():
    ...
    lable0 = Label(root, text='客戶管理', bg='pink', font=('微軟雅黑', 50)).pack()  # 上

    Button(root, text='新增', font=('微軟雅黑', 15), width=10, height=2, bg='yellow', command=insert_CusInfo).place(x=400, y=250)
    Button(root, text='查詢', font=('微軟雅黑', 15), width=10, height=2, bg='yellow', command=search_CusInfo).place(x=400, y=350)
...
def insert_CusInfo():
    ...
    lable1 = Label(win, text='新增客戶資訊:', font=('微軟雅黑', 20)).place(x=30, y=50)
...
def search_CusInfo():
    ...
    lable1 = Label(win, text='查詢客戶資訊:', font=('微軟雅黑', 20)).place(x=30, y=50)
    ...
    if a_name.get() == '':
        if b_name.get() == '全部':
          sql = "SELECT * FROM 客戶"
            ...
            try:
                sql = "SELECT * FROM 客戶 WHERE 客戶姓名='%s'"%(b_name.get())
                Button(win,text='刪除',font=('宋體', 12), bg='yellow', width=10,command=add_fixed).place(x=725,y=195)
        ...
        if a_name.get() == '全部':
          sql = "SELECT * FROM 客戶"
            ...
            try:
                sql = "SELECT * FROM 客戶 WHERE [身分證字號/統一編號]='%s'"%(a_name.get())
                Button(win,text='刪除',font=('宋體', 12), bg='yellow', width=10,command=add_fixed).place(x=725,y=195)
                #Button(win,text='編輯',font=('宋體', 12), bg='yellow', width=10,command=update).place(x=725,y=195)
            except Exception as ex:#更詳細的找錯誤原因
                print(ex)
